...
import { yupResolver } from '@hookform/resolvers/yup';
  ...
  const CreateAnimalForm = forwardRef(
    (
      { title, showOkBtn = true, onSuccess, onError }: Props,
      ref
    ) => {
      const {
        handleSubmit,
        ...
        control,
        formState: { errors },
      } = useForm<Partial<IAnimal>>({
        defaultValues: {
          name: '',
          predator: false
        },
        resolver: yupResolver(AnimalSchema),
      } as UseFormProps<Partial<IAnimal>>);
      ...
      const submit: SubmitHandler<Partial<IAnimal>> = async (data) => {
        console.log(data);
        reset();
        setIsSending(true);
        if (onSuccess) {
        ...
        >
          ...
          <Flex vertical gap={'small'} style={{ padding: '15px 0' }}>
            ...
            <Controller
              ...
              render={({ field }) => (
                <>
                  <Checkbox value={field.value} onChange={field.onChange}>Хищник</Checkbox>
